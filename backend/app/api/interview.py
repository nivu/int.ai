...
@router.get("/magic-auth", status_code=200)
async def magic_auth(token: str) -> dict:
    """Validate a magic link token and return a Supabase sign-in URL.

    The frontend /interview/start page calls this endpoint with the token
    from the URL, then redirects the browser to the returned signin_url.
    Supabase handles the actual authentication and redirects back to /interview.
    """
    from datetime import datetime, timezone
    from app.services.supabase import supabase as sb
    ...
    # Look up the token
    result = (
        sb.table("interview_magic_tokens")
        .select("id, candidate_email, application_id, expires_at, used_at")
        .eq("token", token)
        .execute()
    )
    ...
    if record["expires_at"] < now:
        raise HTTPException(status_code=410, detail="This link has expired. Please contact the hiring team.")

    # Mark as used immediately (single-use enforcement)
    sb.table("interview_magic_tokens").update({"used_at": now}).eq("id", record["id"]).execute()

    # Generate a Supabase magic sign-in link for the candidate's email.
    # The candidate is redirected here after Supabase authenticates them.
    redirect_to = f"{settings.FRONTEND_URL}/auth/callback?next=/interview"
    ...
    try:
        ...
        signin_url = link_resp.properties.action_link
    except Exception:
        # If generating the link fails, roll back the used_at so the candidate can retry
        sb.table("interview_magic_tokens").update({"used_at": None}).eq("id", record["id"]).execute()
        logger.exception("Failed to generate magic sign-in link for %s", record["candidate_email"])
        raise HTTPException(status_code=500, detail="Failed to generate sign-in link. Please try again.")

    logger.info("Magic auth: issued sign-in link for %s", record["candidate_email"])
